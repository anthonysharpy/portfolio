...
export function GoPage() {
    return (
        <div className="go-page-content">
            ...
            <h2>YayOrNay</h2>
            <p>YayOrNay is a novel dating app built in Go and Vue.js. Most of the time, my friend works on the frontend and I work on the backend, although we do switch it up sometimes. Obviously I'm not really able to share the source code or give away the secret sauce behind our algorithm. But I can speak geeky words at you about it. (Sorry, there are some visual glitches in the iframe below that we are still ironing out, but it looks great on the <a href="https://yayornay.se" rel="noreferrer" target="_blank">actual website</a>.)</p>
            ...
            <p>Anyways, so as part of this we managed to spin up 5,000 concurrent WebSocket connections to the test API. Once we had done that, we ran the actual test, which involved sending one message per WebSocket connection (each of which involved multiple database calls, might I add). Our goal: to send and receive a response for these 5,000 messages in under 10 seconds. The verdict? It completed in 2.5 seconds. Very nice! From this single limited test, it would seem that the server can handle roughly 2,000 WebSocket requests per second. But we have to bear in mind that not all our endpoints are WebSockets, and that obviously a connection over LAN with a test database is not indicative of real-world performance, so we have to take this with a grain of salt. But it gives us confidence that our server could likely handle up to 1,000-5,000 users active at the same time. Sure, it's not the next Facebook, but we do have a clear upgrade path if we need to increase the power of the server, and for $21/month, it's certainly not bad! So long as we stay under our 3 TB monthly data limit that is... although because of that we will probably be hosting things like images and videos elsewhere.</p>
            <p>I also want to shout out how great it's been working with MongoDB on this project. The tags feature in Go lets you easily map your structs to your documents. And you don't even need to create a schema for it. Just insert the data and it's there. Storing non-normalised objects as documents also just makes a lot of sense for a lot of use-cases, and often proves itself to be simpler (and occasionally even faster) than the spaghetti data you can get with SQL databases. All that being said, I have to say that MongoDB is an absolute pain to configure. Want to add a root user? Sorry, you have to shut the database off, disable authentication and restart it first! Want to enable transactions? Sorry, you have to shut the database off, configure a replica set and restart it first! Want to configure a replica set? Sorry, you have to create a keyfile first! Once you start to learn these quirks though, it's really not too bad.</p>
