...
export function GoPage() {
    return (
        <div className="go-page-content">
            ...
            <h2>YayOrNay</h2>
            <p>YayOrNay is a novel dating app built in Go and Vue.js. Most of the time, my friend works on the frontend and I work on the backend, although we do switch it up sometimes. Obviously I'm not really able to share the source code or give away the secret sauce behind our algorithm. But I can speak geeky words at you about it.</p>
            <iframe src="https://yayornay.se"></iframe>
            <p>We were quite impressed the other day to learn just how capable our server is at the moment, even with our lousy, 2-virtual-CPU, 2GB-RAM DigitalOcean hosting. We have an integration tester that spins up a test database and a copy of the API to perform all kinds of whacky tests to make sure everything's working properly. I originally thought about making this integration tester in PowerShell, but in the end, I just made a fully-fledged Go program for it instead, since scripting is slow and unwieldy at the best of times, and the same thing in Go was only a hundred lines of code or so, not including the tests themselves.</p>
            <p>Anyways, so as part of this we somehow managed to spin up 5,000 concurrent WebSocket connections to the test API. Once we had done that, we ran the actual test, which involved sending one message per WebSocket connection (each of which involved multiple database calls, might I add). Our goal: to send and receive a response for these 5,000 messages in under 10 seconds. The verdict? It completed in 2.5 seconds. Very nice! Doing the math, it would seem that the server can handle roughly 2,000 WebSocket requests per second. But we have to bear in mind that not all our endpoints are WebSockets, and that obviously a connection over LAN with a test database is not indicative of real-world performance, so we have to take this with a grain of salt. But it gives us confidence that our server could likely handle up to 1,000-5,000 users active at the same time. Sure, it's not the next Facebook, but we have a clear upgrade path if we need to increase the power of the server, and for $21/month, it's certainly not bad! So long as we stay under our 3 TB monthly data limit that is... although because of that we will probably be hosting things like images and videos elsewhere.</p>
            <p>I also want to shout out how great it's been working with MongoDB on this project. The tags feature in Go lets you easily map your structs to your documents. And you don't even need to create a schema for it. Just insert the data and it's there. Storing non-normalised objects as documents also just makes a lot of sense for a lot of use-cases, and often proves itself to be simpler (and possibly even faster) than the spaghetti data you can get with SQL databases. All that being said, MongoDB is an absolute pain to configure. Want to add a root user? Sorry, you have to shut the database off, disable authentication and restart it first! Want to enable transactions? Sorry, you have to shut the database off, configure a replica set and restart it first! Want to configure a replica set? Sorry, you have to create a keyfile first! Once you start to learn these quirks though, it's really not too bad.</p>

            <h2>Safe(r) concurrent access with getters and setters</h2>
            <p>I had an interview a few years ago for a Go job. I think the interviewers were a little bit sceptical of me, as I had no commercial experience of Go at the time. To be honest, I'm not really sure why they offered to interview me, since I don't think they really had any intention of giving me the job.</p>
