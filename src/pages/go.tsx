...
export function GoPage() {
    return (
        <>
            ...
            <p>I had an interview a few years ago for a Go job. The interviewers were clearly skeptical of me, as I had no commercial experience of Go at the time. To be honest, I'm not really sure why they offered to interview me, since I don't think they had any intention of giving me the job. One of the things they said was <i>"So, well, have you for example used things like concurrency in Go?"</i>. I said that I hadn't, but that I had done it in other languages, and said I didn't imagine it would be that hard. They laughed and said <i>"Yeah, right"</i>.</p>
            <p>So, were they right about concurrency being hard? Well, no, they were just jerks. But I will admit that concurrency is not always easy. If you don't respect it, it <em>will</em> come back and bite you, like an American Bully XL. The same is true for probably any programming language.</p>
            <p>So what are some ways I like to tame the beast that is concurrency? Well, personally, I think implementing getters and setters is actually a perfect way to guard concurrent access. For example:</p>
            <pre>
...
func main() {
    ...
    for i := 0; i < 100; i++ {
        ...
        go func(entryName string) {
            ...
            <p>Let's say we have a slice of type <code>FoodOrder</code>. Each <code>FoodOrder</code> describes something that's been ordered at a restaurant. Let's also define a type <code>Food</code>, which describes the (tasty) outcome of the order.</p>
            <pre>
...
func getNumberOfExpensiveOrders(orders []FoodOrder) int {
    ...
    user.Update() // Write to database.
            ...
            <p>The behaviour is the same; we've just added implemented a method on the User type. There aren't any astonishing advantages to doing it this way. Personally it feels more natural to me, since updating a <code>User</code> record necessarily involves a <code>User</code> object (and so can most naturally be thought of as something that should belong to the <code>User</code> type). You could even take it a step further and argue that <code>BanUser</code> could be a type method. But let's not get ahead of ourselves...</p>
            ...
            <p>Now, you may be thinking to yourself things like <i>"Oh, well I wouldn't have structured it like that", or "Well, I think the issue is that [insert function here] actually belongs in [insert package name here]"</i>. But this is not my point. I'm not saying that there is no way to resolve this cyclical dependency, or that it couldn't have been done a <em>different</em> way. I'm merely saying that cyclical dependencies aren't necessarily the result of <em>bad</em> code. What I want to stress is the fact that Go disallows <em>"good code"</em> with the justification that it somehow avoids <em>"bad code"</em>, which is ironic and, in my opinion, just a <em>bad feature</em>. (Also, I would point out that just about every other mainstream programming language allows cyclical dependencies in one form or another.)</p>
            <p>So we need a way to overcome this dependency problem. Unfortunately Go does not provide any convincing alternatives.</p>
            <p>One solution is to create "interfacing packages" - new packages that facilitate communication between the cyclical packages in such as way as to break the cycle, often using interfaces<Reference number={4}/>. But this involves polluting your project with a perverted mixture of meaningless interfaces and redundant package definitions.</p>
            <p>Another solution is dependecy injection<Reference number={3}/>. This makes a lot of sense for things like database and file handlers, since this allows easy mocking during tests (if you're into that). But in just about every other situation this is again a very messy solution, because it essentially involves reducing every package into an object. This a) makes the code much more complicated, b) introduces a messy assortment of program state since you have all these injected objects being passed around, and c) bastardises the core principles of Go; Go is not an object-oriented language, and should not be treated like one.</p>
            <p>Another approach is to try combining multiple packages into one. This works, although this is probably more of a last-resort, since you'd be abandoning the separation of concerns between your packages (assuming your package structure was justified to begin with).</p>
            <p>The only worthwhile solution I've found (although I'm still not fond of it) is to break the code into separate packages. But how do we do that without making our project structure messy? Let me show you!</p>
            ...
            <p>We can refactor this into:</p>
            <pre className="normal-pre">{`/users/users.go\n/users/userstypes/userstypes.go`}</pre>
            <p>userstypes becomes a new package. conversations can use it by importing <code>"myapp/m/v2/users/userstypes"</code>. And voilà! Not only do we keep a tidy project structure, but we have broken the cyclical dependency.</p>
