export function CCPlusPlusPage() {
    return (
        <>
            ...
            <p>C++ is not something I have used "commercially" or something I try and keep up-to-date with. But I am familiar with it and I know that some companies do use it for part of their tech stack, which is why I'm mentioning it here!</p>
            <h2>GNU Compiler Collection contributions</h2>
            <p>A few years ago I contributed some bug fixes for the GNU C++ compiler. Although it was quite arguably the hardest thing I've ever done, I won't blabber on about it at length, but you can see some examples below of the kind of work I did.</p>
            ...
            <p>The entire thing was a partial success and a partial failure. It was a success in that the bot did get better at killing pigs. It was also a success in that I had a lot of fun and learned a lot. But at the same time it was a failure because:</p>
            <ul>
                ...
                <li>Since it was a game and I had only one computer, only one simulation could run at once. Ideally you want to be running hundreds or thousands of simulations at once.</li>
                <li>The mathematical calculations were done on the CPU, not the GPU, which limited its performance (in my defence this was before libraries like PyTorch became mainstream).</li>
            </ul>
            <p>This is what the training looked like. As you can see I made a cheeky windows API UI to go with it, which helped me debug what was going on. The coloured squares show how the bot perceives the three different primary colours (this is ultra cool because you can literally see it learning to recognise corners and lines). The bigger boxes show how the bot sees the world.</p>
            <img src="images/minecraft_bot_1.png"></img>
            <p>Here's a video of the bot in action. As you can see, it does kill pigs. It also exhibits an interesting number of adaptations. For example, it has a propensity for looking at the floor, which must be so that it can keep an eye on the pigs. It also jumps when it sees a pig. This must be because jumping increases your damage in Minecraft, letting it kill more pigs. Lastly, when there are no pigs on the screen, the bot walks around the room, trying to find some.</p>
            ...
            <p>Another project I worked on in C++ was creating a bot for the online game Talisman Online (this was just for fun, I didn't release it or anything).</p>
            <p>Using Cheat Engine I identified memory locations where important data was stored, such as the player's inventory, health, cursor status, etc. The game used an obfuscation technique where every new version of the game would scramble the pointer offsets of the data (this was not due to the actual code having changed since no new features nor bug fixes have been added to the game for a very long time). To make sure the bot worked with each new version I had to implement some memory scanning procedures in C++ that would look for a pattern of memory, and from there, extract the value that I wanted.</p>
            <p>Once I could read the program's memory I was able to use the Windows API to create routines that would perform keypresses. I also made a rudimentary API for the game using the Windows API.</p>
            <p>The bot was really cool, and it was awesome being able to explore things and places I'd never seen in a game I started playing when I was just a kid!</p>
            ...
            <p>It's sort of really bad, but considering I had only just turned 14 at the time, it's actually pretty cool.</p>
